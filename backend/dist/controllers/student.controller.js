...
const completeEnrollment = async (req, res) => {
    ...
    try {
        ...
        logger_1.logger.info('Complete enrollment request received:', {
        ...
        if (validationErrors.length > 0) {
            ...
            logger_1.logger.warn('Student enrollment validation failed:', {
        ...
        const existingUserByEmail = await models_1.default.User.findOne({
            where: models_1.default.sequelize.where(models_1.default.sequelize.fn('LOWER', models_1.default.sequelize.col('email')), models_1.default.sequelize.fn('LOWER', trimmedEmail)),
            transaction
        });
        // For phone: normalize and compare
        // Get all users with phone numbers and check normalized phone numbers
        const allUsersWithPhone = await models_1.default.User.findAll({
            where: {
                phone: { [sequelize_1.Op.ne]: null }
            },
        ...
        const existingUserByPhone = allUsersWithPhone.find(user => {
            ...
            if (matches) {
                logger_1.logger.info(`Duplicate phone found: Input phone "${trimmedPhone}" (normalized: "${normalizedPhone}") matches existing user ${user.id} with phone "${user.phone}" (normalized: "${userNormalizedPhone}")`);
            }
            return matches;
        });
        // Log for debugging
        if (existingUserByEmail) {
            logger_1.logger.info(`Duplicate email found: "${trimmedEmail}" matches existing user ${existingUserByEmail.id} (${existingUserByEmail.name})`);
        }
        ...
        if (existingUserByEmail || existingUserByPhone) {
            ...
            const conflictType = existingUserByEmail && existingUserByPhone
                ...
                : existingUserByEmail
                    ? 'email'
                    : 'phone number';
            logger_1.logger.warn(`Enrollment blocked: Student "${trimmedStudentName}" tried to register with existing ${conflictType}. Existing student ID: ${existingUser?.id}`);
            res.status(409).json({
                status: 'error',
        ...
        if (models_1.default.StudentProfile) {
            ...
            if (finalWalkinDate)
                enrollmentMetadata.walkinDate = finalWalkinDate;
            if (masterFaculty)
            ...
            if (!existingEnrollment) {
                ...
                if (enrollment && (bookingAmount || (emiPlan && emiInstallments && emiInstallments.length > 0))) {
                    ...
                    if (bookingAmount && bookingAmount > 0) {
                        try {
                            await models_1.default.PaymentTransaction.create({
                                studentId: user.id,
                                ...
                                status: PaymentTransaction_1.PaymentStatus.PAID,
                                notes: 'Initial booking amount from enrollment',
                            }, { transaction });
                            logger_1.logger.info(`Created booking payment: studentId=${user.id}, enrollmentId=${enrollment.id}, amount=${bookingAmount}`);
                        }
                        catch (paymentError) {
                            logger_1.logger.error('Error creating booking payment:', paymentError);
                        ...
                        for (const installment of emiInstallments) {
                            if (installment.amount && installment.amount > 0) {
                                try {
                                    ...
                                    logger_1.logger.info(`Created EMI payment: studentId=${user.id}, enrollmentId=${enrollment.id}, amount=${installment.amount}, month=${installment.month}`);
                                }
                                catch (paymentError) {
                                    logger_1.logger.error(`Error creating EMI payment for month ${installment.month}:`, paymentError);
        ...
        logger_1.logger.info(`Transaction committed successfully for student enrollment: userId=${user.id}, email=${email}, batchId=${batchId || 'none'}`);
        // Verify the student was created correctly by querying it back
        try {
            const verifyUser = await models_1.default.User.findByPk(user.id, {
                include: [
                    {
            ...
            if (verifyUser) {
                logger_1.logger.info(`Verified student exists: id=${verifyUser.id}, name=${verifyUser.name}, role=${verifyUser.role}, isActive=${verifyUser.isActive}, hasProfile=${!!verifyUser.studentProfile}`);
            }
            else {
                logger_1.logger.error(`CRITICAL: Student not found after creation! userId=${user.id}`);
            }
        }
        catch (verifyError) {
            logger_1.logger.error(`Error verifying student after creation:`, verifyError);
        }
        res.status(201).json({
            status: 'success',
    ...
    try {
        ...
        for (const filePath of possiblePaths) {
            if (fs.existsSync(filePath)) {
                ...
                logger_1.logger.info(`Found Excel file at: ${filePath}`);
                break;
            }
        }
        if (!excelFilePath) {
            logger_1.logger.error('Course Name Excel file not found in any of the expected locations');
            res.status(404).json({
                status: 'error',
        ...
        try {
            ...
            logger_1.logger.info(`Excel file loaded successfully. Sheets: ${workbook.SheetNames.join(', ')}`);
        }
        catch (parseError) {
            logger_1.logger.error('Failed to read Excel file:', parseError);
            res.status(400).json({
                status: 'error',
        ...
        if (courseNames.length === 0) {
            logger_1.logger.warn('No course names found in Excel file');
            res.status(400).json({
                status: 'error',
        ...
        logger_1.logger.info(`Found ${courseNames.length} unique course names`);
        res.status(200).json({
            status: 'success',
            data: {
                courseNames,
        ...
        logger_1.logger.error('Get course names error:', error);
        res.status(500).json({
            status: 'error',
...
const unifiedStudentImport = async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({
                status: 'error',
        ...
        if (!req.file) {
            logger_1.logger.error('Unified import: No file received');
            res.status(400).json({
                status: 'error',
        ...
        logger_1.logger.info(`Unified import: File received - name: ${req.file.originalname}, size: ${req.file.size}`);
        // Parse Excel file
        let workbook;
        try {
            ...
            logger_1.logger.info(`Unified import: Excel file parsed successfully - sheets: ${workbook.SheetNames.join(', ')}`);
        }
        catch (parseError) {
            logger_1.logger.error('Unified import: Failed to parse Excel file:', parseError);
            res.status(400).json({
                status: 'error',
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                let student = null;
                if (normalizedPhone) {
                    student = await models_1.default.User.findOne({
                        where: {
                ...
                if (!student) {
                    ...
                    logger_1.logger.info(`Created new student: ID=${student.id}, name=${finalName}, phone=${finalPhone}`);
                }
                else {
                ...
                const softwareList = getValue(row, ['softwaresIncluded', 'Softwares Included', 'softwares_included']) ?
                    String(getValue(row, ['softwaresIncluded', 'Softwares Included'])).split(',').map((s) => s.trim()).filter((s) => s.length > 0) : [];
                const parseBatchList = (value) => {
                    if (!value || value === '')
                        return null;
                    if (Array.isArray(value))
                ...
                for (const code of softwareColumns) {
                    ...
                    if (statusValue && (statusValue === 'XX' || statusValue === 'IP' || statusValue === 'NO' || statusValue === 'Finished')) {
                        ...
                        const existing = await models_1.default.StudentSoftwareProgress.findOne({
                        ...
                        if (existing) {
                            ...
                            await models_1.default.StudentSoftwareProgress.create({
                ...
                for (const softwareNameCol of softwareNameColumns) {
                    ...
                    if (statusValue && (statusValue === 'XX' || statusValue === 'IP' || statusValue === 'NO' || statusValue === 'Finished')) {
                        ...
                        const metadata = Object.keys(futureBatchMetadata).some(key => futureBatchMetadata[key] !== null && futureBatchMetadata[key] !== undefined)
        ...
        logger_1.logger.error('Unified import error:', error);
        res.status(500).json({
            status: 'error',
...
exports.unifiedStudentImport = unifiedStudentImport;
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                if (dobValue && !parsedDob) {
                    logger_1.logger.warn(`Row ${rowNumber}: Failed to parse dob from "${dobValue}". Accepted formats: DD/MM/YYYY, YYYY-MM-DD, or Excel serial date`);
                }
                else if (parsedDob) {
...
const getStudentAttendance = async (req, res) => {
    try {
        ...
        const isStudent = req.user.role === User_1.UserRole.STUDENT;
        const isAdmin = req.user.role === User_1.UserRole.ADMIN || req.user.role === User_1.UserRole.SUPERADMIN;
        ...
        if (from || to) {
            where['$session.date$'] = {};
            if (from) {
                where['$session.date$'][sequelize_1.Op.gte] = new Date(from);
            }
            if (to) {
                const toDate = new Date(to);
                toDate.setHours(23, 59, 59, 999); // End of day
                where['$session.date$'][sequelize_1.Op.lte] = toDate;
            }
        }
...
const downloadUnifiedTemplate = async (req, res) => {
    try {
        logger_1.logger.info('Download unified template request received');
        if (!req.user) {
        ...
        logger_1.logger.info('Creating unified Excel template with ALL fields...');
        ...
        const orderedSampleData = sampleData.map((row) => {
            const orderedRow = {};
        ...
        logger_1.logger.info(`Creating worksheet with ${allColumns.length} columns. First column: ${allColumns[0]}, Last columns: ${numericCodes.slice(-3).join(', ')}`);
        ...
        logger_1.logger.info(`Worksheet created. Range: ${worksheet['!ref']}, Total columns: ${allColumns.length}`);
...
exports.downloadUnifiedTemplate = downloadUnifiedTemplate;
