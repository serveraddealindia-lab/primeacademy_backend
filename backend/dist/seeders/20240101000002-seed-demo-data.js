...
exports.default = {
    async up(queryInterface) {
        try {
            ...
            const [existingEmployees] = await queryInterface.sequelize.query(`SELECT email FROM users WHERE email IN (${employeePlaceholders})`, {
                replacements: employeeEmails,
                type: sequelize_1.QueryTypes.SELECT,
            });
            const existingEmployeeEmails = new Set(Array.isArray(existingEmployees) ? existingEmployees.map((e) => e.email) : []);
            ...
            if (employeesToInsert.length > 0) {
                ...
                const [insertedEmployees] = await queryInterface.sequelize.query(`SELECT id FROM users WHERE email IN (${employeePlaceholders})`, {
                    replacements: employeeEmails,
                    type: sequelize_1.QueryTypes.SELECT,
                });
                employeeUserIds = Array.isArray(insertedEmployees)
                    ? insertedEmployees.map((e) => e.id)
                    : [];
            }
            else {
                // Get existing employee IDs
                const [existing] = await queryInterface.sequelize.query(`SELECT id FROM users WHERE email IN (${employeePlaceholders})`, {
                    replacements: employeeEmails,
                    type: sequelize_1.QueryTypes.SELECT,
                });
                employeeUserIds = Array.isArray(existing) ? existing.map((e) => e.id) : [];
            ...
            const [existingFaculty] = await queryInterface.sequelize.query(`SELECT email FROM users WHERE email IN (${facultyPlaceholders})`, {
                replacements: facultyEmails,
                type: sequelize_1.QueryTypes.SELECT,
            });
            const existingFacultyEmails = new Set(Array.isArray(existingFaculty) ? existingFaculty.map((f) => f.email) : []);
            ...
            if (facultyToInsert.length > 0) {
                ...
                const [insertedFaculty] = await queryInterface.sequelize.query(`SELECT id FROM users WHERE email IN (${facultyPlaceholders})`, {
                    replacements: facultyEmails,
                    type: sequelize_1.QueryTypes.SELECT,
                });
                facultyUserIds = Array.isArray(insertedFaculty) ? insertedFaculty.map((f) => f.id) : [];
            }
            else {
                const [existing] = await queryInterface.sequelize.query(`SELECT id FROM users WHERE email IN (${facultyPlaceholders})`, {
                    replacements: facultyEmails,
                    type: sequelize_1.QueryTypes.SELECT,
                });
                facultyUserIds = Array.isArray(existing) ? existing.map((f) => f.id) : [];
            ...
            const [existingStudents] = await queryInterface.sequelize.query(`SELECT email FROM users WHERE email IN (${studentPlaceholders})`, {
                replacements: studentEmails,
                type: sequelize_1.QueryTypes.SELECT,
            });
            const existingStudentEmails = new Set(Array.isArray(existingStudents) ? existingStudents.map((s) => s.email) : []);
            ...
            if (studentsToInsert.length > 0) {
                ...
                const [insertedStudents] = await queryInterface.sequelize.query(`SELECT id FROM users WHERE email IN (${studentPlaceholders})`, {
                    replacements: studentEmails,
                    type: sequelize_1.QueryTypes.SELECT,
                });
                studentUserIds = Array.isArray(insertedStudents) ? insertedStudents.map((s) => s.id) : [];
            }
            else {
                const [existing] = await queryInterface.sequelize.query(`SELECT id FROM users WHERE email IN (${studentPlaceholders})`, {
                    replacements: studentEmails,
                    type: sequelize_1.QueryTypes.SELECT,
                });
                studentUserIds = Array.isArray(existing) ? existing.map((s) => s.id) : [];
            ...
            // ============================================
            const [adminResult] = await queryInterface.sequelize.query(`SELECT id FROM users WHERE email = 'admin@primeacademy.com' LIMIT 1`, { type: sequelize_1.QueryTypes.SELECT });
            ...
            const [batchResults] = await queryInterface.sequelize.query(`SELECT id FROM batches WHERE title IN (${batchTitles.map(() => '?').join(',')}) ORDER BY createdAt DESC LIMIT 5`, {
                replacements: batchTitles,
                type: sequelize_1.QueryTypes.SELECT,
            });
            const batchIds = Array.isArray(batchResults) && batchResults.length > 0
                ? batchResults.map((b) => b.id)
                : [];
    ...
    async down(queryInterface) {
        ...
        await queryInterface.bulkDelete('users', {
            email: {
                [sequelize_1.Op.in]: demoEmails,
            },
        });
        await queryInterface.bulkDelete('batches', {
            title: {
                [sequelize_1.Op.like]: '%Batch 1%',
            },
        });
        console.log('✓ Removed demo data');
    },
};
//# sourceMappingURL=20240101000002-seed-demo-data.js.map