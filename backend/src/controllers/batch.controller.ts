  ...
  try {
    ...
    const candidates = studentsWithMatchingSoftware.map((student) => {
      ...
      const conflictingEnrollments = existingEnrollments.filter((enrollment) => {
        ...
        if (!enrollment.batch) return false;
        if (enrollment.batch.id === batchId) return false; // Exclude current batch
        if (enrollment.batch.status === 'ended' || enrollment.batch.status === 'cancelled') return false;
      ...
      const conflictingSessions = existingSessions.filter((session) => {
        if (!session.batch || !session.batch.enrollments) return false;
        return session.batch.enrollments.some((enrollment: any) => enrollment.studentId === studentId);
      });
...
export const assignFacultyToBatch = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const normalizedIds = req.body.facultyIds.map((id: any) => Number(id));
