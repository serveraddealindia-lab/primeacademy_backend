...
export const getPayments = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    try {
      ...
      // PRIMARY QUERY: Always try to include student data first
      payments = await db.PaymentTransaction.findAll({
        ...
        include: [
          { 
            ...
            required: false,  // LEFT JOIN to include payments even if student missing
            include: [
              {
                model: db.StudentProfile,
                as: 'studentProfile',
      ...
      // Validate that student data is present and log warnings
      payments.forEach((payment: any) => {
        if (!payment.student) {
          logger.warn(`Payment ID ${payment.id} has no student association. studentId: ${payment.studentId}`);
        } else if (!payment.student.name) {
          logger.warn(`Payment ID ${payment.id} student has no name. studentId: ${payment.studentId}, email: ${payment.student.email}`);
        }
      });
